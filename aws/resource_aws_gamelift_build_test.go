...
func TestAccAWSGameliftBuild_basic(t *testing.T) {
	...
	bucketName := fmt.Sprintf("tf-acc-bucket-gamelift-build-%s", acctest.RandString(8))
	roleName := fmt.Sprintf("tf_acc_role_%s", acctest.RandString(8))
	policyName := fmt.Sprintf("tf_acc_policy_%s", acctest.RandString(8))
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccAWSGameliftBuildBasicConfig(buildName, bucketName, roleName, policyName),
			...
			{
				Config: testAccAWSGameliftBuildBasicUpdateConfig(buildName, bucketName, roleName, policyName),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckAWSGameliftBuildExists("aws_gamelift_build.test", &conf),
					resource.TestCheckResourceAttr("aws_gamelift_build.test", "name", buildName),
					resource.TestCheckResourceAttr("aws_gamelift_build.test", "operating_system", "AMAZON_LINUX"),
...
func testAccAWSGameliftBuildBasicConfig(buildName, bucketName, roleName, policyName string) string {
	return fmt.Sprintf(`
...
`, buildName, bucketName, roleName, policyName)
}

func testAccAWSGameliftBuildBasicUpdateConfig(buildName, bucketName, roleName, policyName string) string {
	return fmt.Sprintf(`
resource "aws_gamelift_build" "test" {
  name = "%s"
  operating_system = "AMAZON_LINUX"
